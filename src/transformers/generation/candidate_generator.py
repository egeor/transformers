...
class AssistedCandidateGenerator(CandidateGenerator):
    ...
    def __init__(
        ...
        if hasattr(assistant_model, 'tmp_kv_cache'):
             assistant_kwargs["past_key_values"] = assistant_model.tmp_kv_cache
             self.num_assistant_tokens = assistant_model.generation_config.num_assistant_tokens 
    ...
    def _update_past_and_masks(self, input_ids: torch.LongTensor, remove_from_pkv: int = 0) -> bool:
        ...
        if has_past_key_values:
            ...
            if hasattr(self.assistant_model, 'tmp_kv_cache'):
                has_past_key_values = True
                new_cache_size = input_ids.shape[-1] - 1 - remove_from_pkv
                self.assistant_kwargs["past_key_values"] = _crop_past_key_values(
                    self.assistant_model, self.assistant_model.tmp_kv_cache, new_cache_size - 1
                )
                self.assistant_kwargs = _prepare_attention_mask(
                    self.assistant_kwargs, input_ids.shape[-1], self.assistant_model.config.is_encoder_decoder
                )
    ...
    def _generate_candidates(self, generation_args: Dict) -> Tuple[torch.LongTensor, Optional[torch.FloatTensor]]:
        ...
        if hasattr(self.assistant_model, 'tmp_kv_cache'):   
            self.assistant_model.tmp_kv_cache = assistant_output.past_key_values
...
class AssistedCandidateGeneratorWithHelper(CandidateGenerator):
    """
    `CandidateGenerator` class to be used for assisted generation and speculative decoding. This class generates
    candidates through the use of a smaller model. Read the following blog post for more information:
    https://huggingface.co/blog/assisted-generation
    ...
    def __init__(
        ...
        assistant_tokenizer_helper: Optional["PreTrainedTokenizerBase"] = None,
        inputs_tensor: Optional[torch.Tensor] = None,
        logits_processor: "LogitsProcessorList" = None,
    ):
        # Make sure all data at the same device as assistant model
        ...
        # Setup assistant helper
        self.assistant_model_helper = assistant_model_helper
        self.assistant_tokenizer_helper = assistant_tokenizer_helper
        self.target_tokenizer = target_tokenizer
        self.assistant_model_helper.generation_config.eos_token_id = generation_config.eos_token_id
        ...
        self.generation_config.is_assistant = False
    ...
    def _generate_candidates(self, generation_args: Dict) -> Tuple[torch.LongTensor, Optional[torch.FloatTensor]]:
        """Generate candidate sequences using the assistant model."""
        if self.assistant_tokenizer_helper is None:
            assistant_output = self.assistant_model.generate(**generation_args, assistant_model=self.assistant_model_helper, **self.assistant_kwargs)
        else:
            assistant_output = self.assistant_model.generate(**generation_args, assistant_model=self.assistant_model_helper, tokenizer=self.target_tokenizer, assistant_tokenizer=self.assistant_tokenizer_helper, **self.assistant_kwargs)
...
class AssistedCandidateGeneratorDifferentTokenizers(AssistedCandidateGenerator):
    ...
    def get_candidates(self, input_ids: torch.LongTensor) -> Tuple[torch.LongTensor, Optional[torch.FloatTensor]]:
        ...
        if hasattr(self.assistant_model, 'tmp_kv_cache'):
            self.assistant_model.generation_config.assistant_confidence_threshold = 0.65
        assistant_output = self.assistant_model.generate(**generation_args, **self.assistant_kwargs)
